...
- All [MathJax-supported TeX commands](https://docs.mathjax.org/en/latest/input/tex/macros/index.html) within inline (`$ $`) and display (`$$ $$`) math mode.
- Tables and footnotes as described in CommonMark's [extended syntax](https://www.markdownguide.org/extended-syntax/).
...
`.ans`                          | [Output Validator](#output-validator)               | Input to the Output Validator
...
The default value of `aggregation` is `sum` for the `secret` group and `pass-fail` for its subgroups. The default value of `require_pass` is an empty sequence.
...
- for test cases with bounded maximum score, `score.txt`,  if produced, must contain a single single non-negative floating-point number.
  The score of the test case is that number. The number can be larger than the test case's inferred maximum score.
...
A path consists of zero or more directory names followed by a directory or file name, with a `/` character separating consecutive names. Each name must conform to the [general requirements](#general-requirements) on directory and file names and the specified test data group or test case must exist.
