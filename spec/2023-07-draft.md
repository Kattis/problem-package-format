...
The submission's filename is passed as the first command-line argument, followed by the whitespace-separated tokens in `test.args`, if the file exists.

The folder `test.files`, if it exists, contains privileged data files available to the submission via file I/O. 
All files in this folder must be copied into the submission's working directory after compiling,
