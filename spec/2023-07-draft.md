...
Input validators can be specified as VIVA-files (with file ending `.viva`), Checktestdata-file (with file ending `.ctd`), or as a [program](#programs).
...
It must be provided as the directory `output_validator/`, and may contain `build` and `run` scripts (see [Programs](#programs)).
