...
There are a number of different kinds of programs that may be provided in the problem package: submissions, input validators, output validators, and visualizers.
...
Each output validator and visualizer must be run with a working directory that contains the submitted files and any compiled binaries of the submission being validated.
...
The validator may create one or more image files in the feedback directory with name `teamimage.ext` and/or `judgeimage.ext`, where `ext` is an appropriate image file format extension. 
The [visualizer](#visualizer) may modify or create these files as well. 
The intent is for the `teamimage` to be displayed to contestants and for `judgeimage` to be a priviledged information used as a debugging aid by contest judges, but the judge system may display or ignore these files as it sees fit.
...
A visualizer is an optional [program](#programs) that is run after every invocation of the output validator in order to generate images illustrating the submission output. 
...
It must be provided as the directory `visualizer/`.
It must either be written in C++ or Python 3,
or must provide a `build` or `run` script as specified [above](#programs).

All files written to the feedback directory by the output validator are accessible to the visualizer. 
The visualizer may overwrite or create image files in the feedback directory with name `teamimage.ext` or `judgeimage.ext`, where `ext` is an appropriate image file format extension. 
...
To achieve that behavior, create groups `secret/subtask1`, `secret/subtask2`, ... and set the `score` value in the `secret/subtaskX/testdata.yaml`.
