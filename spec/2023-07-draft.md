...
Metadata abut the example submissions are provided in a UTF-8 encoded YAML file named `submissions.yaml` placed directly in the `submissions/` directory.
The top level keys in `submissions.yaml` are. globs matching example submissions.
E.g. `accepted/*` would match any submission in the `submissions/accepted/` directory.
