...
- All file names and directories included in the package must match the regexp
  ```regex
...
- Auxiliary files and directories beginning with a period (such as `.gitignore` and `.git/`) are allowed in the problem package for convenience when interoperating with external tools. However, the judging system shall behave as if all such files and directories were removed before the system interacts with the package (and in particular, no test case, submission, or other file integral to the package should include a leading period.)
