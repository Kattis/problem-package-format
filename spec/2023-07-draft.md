...
Groups and subgroups with `sum` or `min` aggregation and every test case in these groups have a maximum possible score.
...
The default `score` for other test data groups with `sum` or `min` aggregation is inferred from the `score` value of its parent and siblings,
...
The submission score is the score of the `secret` group. It is a judge error if the score of any group or subgroup exceeds its maximum score.
