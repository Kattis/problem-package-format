...
For scoring problems, the concept of verdict is less defined.
In fact, some judging systems may choose to not even present a final verdict for a submission, instead only present for example the score and the verdicts on those test cases the submission failed.

The scoring behaviour is configured by the following flags under `grading` in `testdata.yaml`:
...
- The _sample_ group: `score` is 0, `scoring` is `sum`.
...
They are chosen to minimize the configuration needed for a typical scoring pproblem that might appear in olympiad-style contests such as the International Olympiad in Informatics.
There, points are given for passing all test cases in groups of testcases called subtasks.
To achieve that behaviour, create test data groups `secret/subtask1`, `secret/subtask2`, ... and set the `score` value in the `secret/subtaskX/testdata.yamlsecret/`.
