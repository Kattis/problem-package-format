...
Sample test cases for interactive problems may specify an interaction protocol as a text file with the extension `.interaction` demonstrating the communication between the submission and the output validator.
When specified, the `.interaction` file should be displayed in the problem statement, replacing the `.in` and `.ans` (or `.out`) files —
the `.in`, `.ans`, and `.out` files for that test case **must not** be displayed to teams:
not in the problem statement, nor as part of sample input download.

Additional sample interaction protocols may be defined by creating an `.interaction` file in the directory `data/sampe/statement/` without corresponding `.in` and `.ans` files.
