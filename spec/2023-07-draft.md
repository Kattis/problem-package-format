...
In some problems, the author may want to provide samples in the statement for demonstrative purposes but does not want them to be run.
...
This option may only be used at the root; if omitted, its value defaults to `true`.
The samples should still be validated by input and output validators.
Note that the keys `input_validator_args` and `output_validator_args` in `testdata.yaml`, as described above,
