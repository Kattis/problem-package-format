...
If one of these files exists, its contents replaces that of the the same file without `.statement` for purposes of the problem statement.
Note that `.statement` files are not validated.
...
For interactive problems the `.in` and `.ans` pairs typically do not meaningfully capture how users are expected to interact with the output validator in such problems.
Therefore, it is adviced to provide samples for the problem statement in one of the following two forms:
- An `.in.statement` and `.ans.statement` file as explained before.
...
Note that you should choose one of these two options and not both.
...
For multi-pass problems the `.in` and `.ans` pairs typically do not meaningfully capture how users are expected to interact with the output validator in such problems.
Therefore, it is adviced to provide samples for the problem statement in one of the following two forms:
- An `.in.statement` and `.ans.statement` file as explained before.
- An `.interaction` file that contains lines starting with `<` and `>`, like for interactive problems.
  Passes are separated by a line containing `---` (three dashes).
  When the problem is not interactive, simply start each pass by a number of lines starting with `<`, containing the sample input, followed by some lines starting with `>`, containing the sample answer.
...
By default, the files in `data/sample` are available for download, except for `.interaction` files.
Note that, files with `.statement` replace those without the additional extension, e.g., `.in.statement` is renamed to `.in` and replaces it for the download.
This behavior can be further customized by creating files with the additional extension `.download` like `.in.download`.
If one of these files exists, its contents replaces that of the the same file without `.download` extension for the download.
Note that `.download` files are not validated.

#### Validation

All `data/sample/*.in` files are input-validated.
For non interavtive and non multi-pass problems all test cases in `data/sample` must be accepted by the output validator, with the `.ans` file used as both the `answer_file` and `team_output`.
All other files are not validated in any way.

Note that since `.statement` files are not validated this allows to display invalid test cases to the user.
This feature should only be used if necessary, for example if all valid testcases are too large for a meaningfull sample. 
