...
- All directory names inside the package must match the regexp
  ...
  ^[a-zA-Z0-9]([a-zA-Z0-9_-]{0,253}[a-zA-Z0-9])?$
  ```
  i.e., they must be of length at least 1, at most 255, consist solely of lower- or uppercase letters a–z, A–Z, digits 0–9, dash, or underscore,
...
E.g. if the file `data/secret/huge.in` exists then the directory `data/secret/huge/` must not, and vice versa.
