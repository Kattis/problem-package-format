...
| `input_validator_args`  | String or map of strings to strings | empty string            | Inherited      | Arguments passed to each input validator for this test data group. If a string then those are the args that will be passed to each input validator for this test data group. If a map then each key is the name of the input validator and the value is the args to pass to that input validator for this test data group. Validators not present in the map are run without args.
