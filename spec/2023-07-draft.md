...
The validator may create one or more image files in the feedback directory with name `testcase.ext`, where `ext` is an appropriate image file format extension. 
The [visualizer](#visualizer) may overwrite or create these files as well. 
The judge system may display or ignore these files as it sees fit.
...
The visualizer may overwrite or create image files in the feedback directory with name `testcase.ext`, where `ext` is an appropriate image file format extension. 
It must not write to `score.txt`, `teammessage.txt`, or any other files in the feedback directory other than those of the form `testcase.ext`.
