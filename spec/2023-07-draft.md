...
A contest system may support a superset of ASCII, including multi-byte encodings such as UTF-8, so long as the default output validator conforms to this specification whenever both the submission output and answer file consist only of the 128 standard ASCII characters.
...
Otherwise, the submitted output token is accepted if it matches the answer file token up to case. The default output validator must treat uppercase ASCII letters `A`--`Z` as equivalent to their lowercase counterparts whenever both the submission ouput and answer file consist only of the 128 standard ASCII characters. A contest system that supports a superset of ASCII may implement more lenient, locale-specific case-insensitive string comparison.
