...
The top level keys in `submissions.yaml` are `language_versions` and globs matching example submissions.
...
This metadata is informational and provides hints about the development environment used during problem package preparation. It can be useful for debugging e.g. problems encountered when compiling example submissions in the future.
