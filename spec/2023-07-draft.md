...
All files written to the feedback directory by the output validator are accessible to the visualizer. If the output visualizer does not have sufficient to generate an adequate visualization using only the command line arguments, it is recommended that the output validator writes the necessary data to the feedback directory.
