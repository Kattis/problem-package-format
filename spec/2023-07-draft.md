...
Therefore, it is advised to provide samples for the problem statement in one of the following two forms:
...
Note that neither `.statement` nor `.download` files are validated which allows to display invalid test cases to the user.
This feature should only be used if necessary, for example if all valid test cases are too large for a meaningful sample. 
If these files do represent valid output it is advisable to link them as testcases in `data/valid_output/`, see [Valid Output](#valid-output).
