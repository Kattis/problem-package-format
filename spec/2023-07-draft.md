...
Note that you can not combine `.statement` and `.interaction` files.
...
By default, the `.in` and `.ans` files in `data/sample` are available for download.
...
For non interactive and non multi-pass problems all test cases in `data/sample` must be accepted by the output validator, with the `.ans` file used as both the `answer_file` and `team_output`.
If the problem is neither interactive nor multi-pass and `.in.statement` does not exits then `.ans.statement` is assumed to be a valid submission output for `.in`.
