...
* Natural language keys (for example in the `name` map in `problem.yaml`) must be given as 2-letter [ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) code if it exists, otherwise as a 3-letter code from ISO 639.
...
* All floating-point numbers (in any files, including in the problem package or submission output, that are parsed by contest system tools or the default output validator) must be given in decimal and may use scientific notation.
  More specifically, floating-point numbers must satisfy the following grammar, which accepts formatted floating-point output from most major programming languages:
  ```regex
  sign         [+-]
  digit        [0123456789]
  expIndicator [Ee]
...
There are a number of different kinds of programs that may be provided in the problem package: submissions, input validators, output validators, and output visualizers.
...
Validators and visualizers, but not submissions,
...
but it must not contain any of the files described in the "Test data" section.
...
Each output validator and output visualizer must be run with a working directory that contains the submitted files and any compiled binaries of the submission being validated.
...
| name                   | String or map of strings                                  |                                                          | Required. The name of the problem in each of the languages for which a problem statement exists. See [details below](#name).
| uuid                   | String                                                    |                                                          | Required. UUID identifying the problem, see [below](#uuid-and-version) for usage.
| version                | String                                                    |                                                          | See [below](#uuid-and-version) for usage.
| credits                | String or map with keys as defined below.                 |                                                          | Who should get credits for creating this problem. See [details below](#credits).
| source                 | String, a sequence, or a map as [defined below](#source). |                                                          | The problem set from which the problem originates. See [details below](#source).
| license                | String                                                    | `unknown`                                                | License under which the problem may be used. Value has to be one of the ones [defined below](#license).
| rights_owner           | String                                                    | Value of `credits.authors`, if present, otherwise value of `source`. | Owner of the copyright of the problem. Required to have a value if license is something other than `unknown` or `public domain`. Forbidden if license is `public domain`.
| limits                 | Map with keys as defined below                            | see [definition below](#limits)                          | Time, memory possibly other limits to be imposed on submissions.
| keywords               | Sequence of strings                                       |                                                          | List of keywords.
| languages              | String or sequence of strings                             | All                                                      | List of programming languages that this problem should be restricted to or the string `all`.
| constants              | map of strings to int, float, or string                   |                                                          | Global constant values used by the problem. See [definition below](#constants).
...
Two values that are listed as incompatible must not both be in the sequence.
...
The `source` key contains one or more source entries that this problem was from.
...
If the is only a single source entry, it can be specified directly as value of `source`,
...
| `.out`                          | Sample Output        | Example correct (AC) output to sample input |
...
Unlike in non-interactive problem, `.in` and `.ans` files in interactive problems **must not** be displayed to teams:
not in the problem statement, nor as part of sample input download.
Instead, all sample test cases must provide an interaction protocol as a text file with the extension `.interaction` for each sample demonstrating the communication between the submission and the output validator,
...
Test cases in the `sample` test data group may optionally include an `.out` file. 
This file contains example correct output for the sample input in the corresponding `.in` file. 
This output is displayed in the problem statement (except if the problem statement is in PDF format — [See Problem Statements](#problem-statements)) and the sample output file must pass output validation given the test case's `.ans` file.

The `.out` file may be omitted, in which case the `.ans` file will be used as the `.out` file.
In this case, the `.ans` file must pass output validation.
...
One example of the uses for this are problems that require probabilistic methods to solve.
...
The directory `invalid` can be organized into a tree-like structure similar to `secret` and contain flags in `testdata.yaml` files that are passed to the validators.
...
The validator may create one or more image files in the feedback directory with name `teamimage.ext` and/or `judgeimage.ext`, where `ext` is one of: `png`, `jpg`, `jpeg`, or `svg`.
The [output visualizer](#output-visualizer) may modify or create these files as well, and the output validator may create files in the feedback directory containing metadata that helps the visualizer in this task. 
...
The visualizer may overwrite or create image files in the feedback directory with name `teamimage.ext` or `judgeimage.ext`, where `ext` is one of: `png`, `jpg`, `jpeg`, or `svg`.
...
The scoring behavior is configured for each test data group by the following flags in the `scoring` dictionary of its `testdata.yaml`:
...
The default `score` for other test data groups is inferred from the `score` value of its parent and siblings, as is the maximum score of each test case in the group:
...
The score of a failed test case is always 0. By default, the score of an accepted test case is its maximum score, computed as described above. 
...
It is a judge error if an output validator accepts a test case in an unbounded group and does not produce a `score.txt`. It is also a judge error if an output validator produces a `score.txt` for a test case in a group with `passs-fail` aggregation.
...
If the submission receives an accept verdict for all test cases in the group and its subgroups, the score of the group is equal to its maximum possible score. 
...
### Required Depedendent Groups

A group may specify that it should only be scored if a submission is accepted for another test case, or all test cases in another test data groups and their dependencies. Otherwise, none of the group's test cases are judged and the group score is 0.

The paths of these required test cases or groups, relative to the `data` folder, are listed under the `require-pass` key. The path of a group, relative to the `data/` folder, must come later lexicographically than the paths of all dependent test cases and groups.
