...
| static_validation      | Map or boolean                      | false                   | Not applicable | Configuration of static validation test data node. See [Static Validation](#static-validation)
...
If a map is specified, it may have two keys `flags`, and in the case of scoring test groups, `score`.
...
It can have the value of `false` meaning there is no static validaton, `true` meaning that static validation is enabled with no score defined and no flags.
