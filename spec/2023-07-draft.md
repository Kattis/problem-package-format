...
The language of a submission program is determined by the `language` key in `submissions.yaml` if present; otherwise, by comparing the file extensions of the submission file(s) to those specified in the [languages table](../appendix/languages.md). If a single language can't be determined, building fails. [Included files](#included-files), if any, must be copied into the submission folder before building the submission.
...
Each submission must be run with a working directory that contains (a copy of) the submitted files, any compiled binaries and compilation byproducts, as well as any [included files](#included-files), and nothing else. In particular the working directory must not contain any of the [test data](#test-data) files.
...
Each output validator and output visualizer must be run with a working directory that contains the submitted files and any compiled binaries of the submission being validated. (Note in particular that this working directory does *not* contain the files in the program directory of the output validator or visualizer.)
...
| ------------------------------------------------- | --------------------------------------------- | -------------------
| [problem_format_version](#problem-format-version) | String                                        | Required
| [type](#type)                                     | String or sequence of strings                 | `pass-fail`
| [name](#name)                                     | String or map of strings                      | Required
| [uuid](#uuid-and-version)                         | String                                        | Required
| [version](#uuid-and-version)                      | String                                        |
| [credits](#credits)                               | String or map with keys as defined below      |
| [source](#source)                                 | String, a sequence, or a map as defined below |
...
| [keywords](#keywords)                             | Sequence of strings                           |
| [languages](#languages)                           | String or sequence of strings                 | `all`
| [constants](#constants)                           | Map of strings to int, float, or string       |
...
| static_validation      | Map                                 | [See Static Validation](#static-validator) | Not applicable | Configuration of static validation test data node.
...
`data/sample` must not contain test groups. Also, it does not contain any test
cases (for example when the samples shown in the PDF are not actually valid cases).

#### Samples shown in the problem statement
By default, the `.in` and `.ans` pairs from `data/sample` are shown in the
problem statement.
This can be customized by creating the `data/sample/statement` directory. If
this directory exists, _only_ those testcases present in this directory are shown, and the
test cases in `data/sample` are ignored for purposes of the problem statement.
...
- A `.in`, `.ans`, and `.out` file, of which the `.in` and `.out` file are shown, and the
  `.ans` file is used for validation.
...
When `data/sample/download` exists, the files in there are available for
download.
If this directory does not exist, the contents of `data/sample/statement` are
...
The `data/sample/download` directory may contain anything, including e.g. a
testing tool for interactive problems, and is not validated. Testing tools may
warn when test cases that are listed in `data/sample` or `data/sample/statement`
do not appear in `data/sample/download`, or when `data/sample/download` contains
test cases that do not appear in one of the two other locations.

#### Validation
All `data/sample/*.in` files are input-validated.

When `data/sample/statement/` does not exist,
`data/sample/*.ans` is output validated (i.e. must be accepted).
...
Files in `data/sample/download` are never validated, although tooling may warn
when there are inconsistencies between `download`, `statement`, and
`data/sample/` itself.

Validation can be customized by specifying `input_validator_flags` and
`output_validator_flags` in `data/sample/testdata.yaml`.
...
name (e.g., do not match `/`). A test case is matched by the glob pattern if either
...
This is supported for backwards compatibility and is not recommended for
normal usage.
...
A static validation test case is defined within a group's `testdata.yaml` file by specifying the map `static_validation`.
The map can have two keys `flags`, and in the case of scoring test groups, `score`.
The key `flags` maps to a string which represents the additional arguments passed to the static validator in this group's static validation test case.
...
When invoked the static validator will be passed at least two command line parameter.

The validator should be possible to use as follows on the command line:
```sh
<static_validator_program> submission_dir feedback_dir [additional_arguments]
```

The meaning of the parameters listed above are:

- submission_dir:
  a string specifying the absolute path to a directory containing a copy of the submission
