...
* All file names for files included in the package must match the regexp
  ```regex
  (?=.{2,255}$)^[a-zA-Z0-9][a-zA-Z0-9_.-]*[a-zA-Z0-9]$
  ```
...
| credits                | String or map with keys as defined below.                 |                                                          | Who should get credits for creating this problem. See below for details.
...
| limits                 | Map with keys as defined below                            | see definition below                                     | Time, memory possibly other limits to be imposed on submissions.
