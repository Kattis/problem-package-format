...
* All floating-point numbers (in the problem package or submission output that are parsed by contest system tools or the default output validator) must be given in decimal and may use scientific notation.
  ...
  ```
The systems parsing these floating-point numbers may impose reasonable limits on the number of digits, but must support at least 30 digits before and after the decimal point. They should use an internal representation with at least 52 bits of mantissa precision and make a best effort to parse floating-point numbers to their closest representable values.
