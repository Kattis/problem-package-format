...
| author                                  | String, sequence or map of strings                          |                                                         | Who should get author credits. This would typically be the people that came up with the idea, wrote the problem specification and created the test data. This is sometimes omitted when authors choose to instead only give source credit, but both may be specified. See below for details.
...
Authors are specified as a sequence of either strings with their full names (optionally with an email formatted as "Full Name \<fullname@problem.example\>"), or maps with keys `name` and `email`.
The examples
```yaml
...
If there is only a single author, the value of `author` may be a single string:
```yaml
...
Names must consist only of ASCII lowercase letters (`a`--`z`) and digits (`0`--`9`). 
*Constant sequences* are tokens (regex words) of the form `XnameX` or `XnameXformatX`, where `name` and `format` are nonempty strings of ASCII lowercase letters or digits. 
...
The `name` in every constant sequence must be a valid constant name, and `format` must consist of exactly one of the following characters: `e`, `s`, or `c`. 
If a `format` is not specified, the constant sequence is replaced verbatim by the corresponding value. Format flags have the following effects:
| Flag                      | Meaning
| ------------------------- | -------
...
In other words, for any two test cases, if the contents of their `.in` and `.args` files,
and `.files` folder are equivalent, then the input of the two test cases is equivalent. 
Additionally, for any two test cases, if their input,
...
For that purpose, a judge system could choose to reuse the result of a previous run, or re-run the equivalent test case.
...
For a test case with base name `test`, the file `test.in` (if it exists) is piped to the submission as standard input. 
The submission's filename is passed as the first command-line argument, followed by the whitespace-separated tokens in `test.args` (if this file exists).

`test.files`, if it exists, must be a folder, and contains privileged data files available to the submission via file I/O. 
All files in this folder are copied into the submission's working directory after compiling but before executing the submission (possibly overwriting the compiled submission file or included data in the case of name conflicts).
...
See [Grading](#grading "wikilink") for more details.
...
| run_samples            | Boolean                             | true                    | Signifies whether samples should be run, see below for details.
...
| rejected                                          | Is not accepted for any reason for at least some test file                                                    |
...
The scoring behavior is configured by the following flags under `grading` in `testdata.yaml`:
