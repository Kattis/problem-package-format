...
| language_versions      | Map with keys as defined below                            |                                                          | Optional metadata about the development environment used during package preparation.
...
The `language_versions` key maps to a dictionary from language strings given in the [language list](#languages) to maps with the keys as defined below:

| Key             | Type    | Default               | Comment
| --------------- | ------- | --------------------- | -------
| command         | String  |                       | Command to run when compiling/interpreting submissions written in this language
| args            | String  | Empty string          | Argument list for `command`. The special token `{files}` may be used to denote where to include the source file list within the argument string.
| version_command | String  | `<command> --version` | Command to run to get the compiler/interpreter version.
| version         | String  |                       | Expected output from running the `version_commmand`.

This optional metadata is informational and provides hints about the development environment used during problem package preparation. It can be useful for debugging e.g. problems encountered when compiling example submissions or custom validators in the future.

An example valid `language_versions` dictionary:
