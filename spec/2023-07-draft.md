...
| `.ans`                          | Output Validator     | Answer file used to validate submission output |
...
Test cases in the `sample` test data group may optionally include an `.out` file. This file contains example correct output for the sample input in the corresponding `.in` file. This output is displayed in the problem statement (except if the problem statement is in PDF format---[See Problem Statements](#problem-statements)) and the sample output file must pass output validation given the test case's `.ans` file.

For problems that use a default output validator where the `.ans` file is the output of a trusted judge reference solution, the `.out` file may be ommitted, in which case the contents of the `.ans` file will be used as the sample output.
