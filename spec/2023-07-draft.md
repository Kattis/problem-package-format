...
| `require-pass`| String or sequence of strings | Other test groups whose test cases a submission must AC in order to receive a score for this test group. See below.
...
A group may specify that it should only be scored if a submission is accepted for all test cases in one or more other test data groups and their dependent groups. Otherwise, none of the group's test cases are judged and the group score is 0.

The paths of these required groups, relative to the `data` folder, are listed under the `require-pass` key. 

Each required group must be either `sample` or a subgroup of `secret`. 

It is an error for required groups to form cyclic dependencies or for a group to require one of its descendents.