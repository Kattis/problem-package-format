...
- Auxiliary files beginning with a period (such as `.gitignore`) are allowed in the problem package for convenience when interoperating with external tools. However, no test case or single-file [program](#program) may begin with a period.
