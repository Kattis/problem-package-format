...
If the preoblem is neither interactive nor multipass and `.in.statement` does not exits then `.ans.statement` is assumed to be a valid submission output for `.in`.
Thus it has to pass output validation `output_validator .in .ans < .ans.statement`.
