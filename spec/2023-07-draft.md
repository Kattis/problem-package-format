...
| name                                    | String or map of strings                                    |                                                         | Required. The names of the problem in the languages that there are problem statements for. See below for details.
...
The set of languages must **exactly** match that for which a problem statement exists.

If only a single problem statement exists, this may be a string with the names of the problem in that language (but a map with a single key is allowed).
