...
For pass-fail problems, the verdict of a submission is accepted if and only if every test case in the `secret` group and its subgroups (and also in the `sample` group, unless `run_samples` is false) is accepted. 
