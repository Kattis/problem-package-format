...
| type                   | string\|sequence[string]                           | `pass-fail`                                              | Type of problem. Values must be from the ones [defined below](#type).
| name                   | string\|map[string => string]                      |                                                          | Required. The name of the problem in each of the languages for which a problem statement exists. See below for details.
| uuid                   | string                                             |                                                          | Required. UUID identifying the problem.
| credits                | string\|map with keys as defined below.            |                                                          | Who should get credits. See below for details.
| source                 | string\|sequence\|map as [defined below](#source). |                                                          | The problem set from which the problem originates. See below for details.
...
| keywords               | sequence[string]                                   |                                                          | List of keywords.
| languages              | sequence[string]\|`all`                            | all                                                      | List of programming languages or the string `all`.
| constants              | map[string => int\|float\|string]                  |                                                          | Global constant values used by the problem. See definition below.
...
| Key               | Type                            | Default | Comments
| ----------------- | ------------------------------- | ------- | --------
| authors           | person\|sequence[persons]       |         | The people who conceptualized the problem.
| contributors      | person\|sequence[persons]       |         | The people who developed the problem package, such as statement, validators, and test data.
| testers           | person\|sequence[persons]       |         | The people who tested the problem package, for example by providing a solution and reviewing the statement.
| translators       | map[string => sequence[person]] |         | The people who translated the statement to other languages. Each key must be an ISO 639 language code.
| acknowledgements  | person\|sequence of persons     |         | Extra acknowledgements or special thanks that do not fit the other categories.
...
| authors    | person\|sequence[person] |                                                                        | Author(s) of submission(s)
...
| `aggregation` | `sum`\|`min` | If sum, the score is the sum of the subresult scores. If min, the score is the minimum of the subresult scores.
