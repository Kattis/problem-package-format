...
It parses the submission output and answer files as UTF-8 (without byte-order mark) and tokenizes the files by splitting on sequences of one or more consecutive Unicode whitespace code points. 
In its default mode, the default output validator then ignores the whitespace and compares the submission output and answer files token by token. If there is a disagreement in the number of tokens in the two files, the validator rejects the submission with a Wrong Answer verdict. The validator may also reject as Wrong Answer any submission output that is unreasonable large (including due to containing unreasonable amounts of whitespace).

The default output validator supports the following command-line arguments to control the above process:

Arguments                    | Description
---------------------------- | -----------
`case_sensitive`             | indicates that comparisons should be case-sensitive (see below for details).
`space_change_sensitive`     | indicates that differences in type or amount of of whitespace should be rejected (see below for details).
...
If a floating-point tolerance has been set, the default output validator will attempt to parse each answer file token as floating-point (see [general requirements](#general-requirements) for details). For each success, the token is compared to the submission output token using the following floating-point semantics. (If no floating-point tolerance has been set, floating-point tokens are [compared as strings](#comparing-string-tokens).)

If the submission output token cannot be parsed as floating-point, the validator returns a Wrong Answer verdict. Otherwise, if `s` is the submission output floating-point value and `a` is the answer file value:
- if an absolute tolerance `ε` has been set, the token is accepted if `|s-a| ≤ ε`;
- if a relative tolerance `ε` has been set, the token is accepted if `s` and `a` are both exactly zero, or if `|s-a|/|a| ≤ ε`;
...
Note that if a floating-point tolerance has been set, the default output validator will parse exact integers as floating-point and apply the above semantics to them. For problems containing a mix of integer and floating-point output, a custom output validator must be used if exact comparison of the integer tokens are required.

#### Comparing String Tokens

If `case_sensitive` is specified, the two tokens must match exactly, i.e. consist of exactly the same code point sequences. 

Otherwise, the submitted output token is accepted if it matches the answer file token up to case. The default output validator must treat uppercase ASCII letters `A`--`Z` as equivalent to their lowercase counterparts. A contest system may additionally implement more lenient, locale-specific case-insensitive string comparison.

#### Whitespace Sensitivity

By default, whitespace in the submission output is ignored by the default output validator after tokenization. As a consequence, the validator is lenient with regard to leading and trailing whitespace, and it treats any sequence of one more whitespace characters in between tokens as equivalent to any other such sequence.

If the `space_change_sensitive` argument is set, the default output validator will instead reject with a Wrong Answer verdict any submission output whose whitespace sequences (including leading and trailing) differ from those in the answer file in type or amount.
