...
Each required group must be either `sample` or a have `pass-fail` aggregation.
The dependent group will only be run if the group being depended on gets its maximum score, that is receives an accepted verdict for all test cases in the group.
