...
### Samples
Sample test cases are used in three places:
...
Some problems require (slightly) different data in each of these cases. We allow
customizing this in `data/sample`, `data/sample/statement`, and `data/sample/download`.
...
For each test case, it must contain one of the following for default or custom
validation (non-interactive) problems:
- A `.in` and `.ans`, which are both shown.
- A `.in`, `.ans`, and `.out`, of which the `.in` and `.out` are shown, and the
  `.ans` is used for validation.
For interactive problems, it must contain one of:
- A `.in` and `.out`, which are both shown, neither of which is validated.
- A `.interaction` file that contains lines starting with `<` and `>`,
  containing an interaction log with output for the interactor (`<`) and
  submission (`>`).
...
available for download, where any `.out` files are renamed to `.ans`
