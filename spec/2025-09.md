...
They are divided into two categories: *submission limits* and *resource guarantees*.
...
Systems are also permitted to enforce a reasonable wall-clock time limit to handle submissions that consume little CPU time (e.g., by sleeping or blocking on I/O).

The `memory` limit specifies the maximum memory a submission may use.

The `output` limit specifies the maximum total size of output a submission may produce, including standard output, standard error, and any files created (when file writing is allowed).

The `code` limit specifies the maximum total size of the source code of a submission.
...
If this wall-clock limit is exceeded (e.g., due to the submission and validator both blocking), the result is a Judge Error.
...
`.out`                          | [Displaying test cases](#displaying-test-cases)     | Example submission output, used instead of `.ans` in statement. Must pass validation
`.interaction`                  | [Displaying test cases](#displaying-test-cases)     | Interaction to be displayed instead of `.in`, and `.ans`
...
#### Interactive and multi-pass problems
...
Multi-pass problems also require a custom output validator; see [Multi-pass validation](#multi-pass-validation).

For both interactive and multi-pass problems, displaying two files typically does not meaningfully capture how users are expected to interact with the output validator.
