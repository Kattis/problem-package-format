...
- All file and directory names included in the package must match the regexp
  ...
  ```
  i.e., they must be of length at least 1, at most 255, consist solely of lower- or uppercase letters a–z, A–Z, digits 0–9, period, dash, or underscore,
  but must not begin with a dash or period.
- Auxiliary files and directories beginning with a dash or period (such as `.gitignore` and `.git/`) or containing invalid characters are allowed in the problem package for convenience when interoperating with external tools. However, the judging system shall behave as if all such files and directories were removed before the system interacts with the package. Package preparation tools are encouraged to warn the user if a file or folder that is ignored in this way appears intended as a test case, submission, or other file integral to the package.
