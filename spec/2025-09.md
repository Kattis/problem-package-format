...
`.out`                          | [Displayed test cases](#displayed-test-cases)       | Example submission output, used instead of `.ans` in statement. Must pass validation
`.interaction`                  | [Displayed test cases](#displayed-test-cases)       | Interaction to be displayed instead of `.in`, and `.ans`
`.in.statement`                 | [Displayed test cases](#displayed-test-cases)       | Used instead of `.in` in the statement
`.ans.statement`                | [Displayed test cases](#displayed-test-cases)       | Used instead of `.and` in the statement
`.in.download`                  | [Downloadable test cases](#downloadable-test-cases) | Used instead of `.in` for downloads
`.ans.download`                 | [Downloadable test cases](#downloadable-test-cases) | Used instead of `.and` for downloads
...
Every subdirectory of `data/secret/` that contain a `test_group.yaml` configuration file is a test data group.
...
`full_feedback`           | Boolean                             | `true` in `sample`, otherwise `false`          | See [Test cases with full feedback](#test-cases-with-full-feedback).
...
We allow customizing which data is used for each purpose with the additional extensions `.statement` and `.download`. See more in [Overriding displayed and downloaded test case files](#overriding-displayed-and downloaded-test-case-files).
...
This can be configured by using `.out`, `.in.statement`, `.ans.statement`, and `.interaction` files.
See more in [Displayed test cases](#displayed-test-cases).
...
By default, the `.in`, `.ans`, and the files in `.files` in `data/sample` are available for download. 
The files available for download can be specified by using `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files. See more in [Downloadable test cases](#downloadable-test-cases).
...
If a test case has `full_feedback` set to `true` (see [Test case configuration](#test-case-configuration)), then by default, the following should be displayed, and available for download to a solver that didn't pass the test case:
  - the given input,
...
The exact files displayed and made available for download can be customized by using `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files. 
See more in [Displayed test cases](#displayed-test-cases). 

The relevant files are not made available if `full_feedback` is set to `false`.

### Validation test data

The `data` directory may contain directories with test cases that must be validated, but is not used when judging a submission.
...
### Overriding displayed and downloaded test case files

This section specifies which optional files may override the default test case data shown to teams or made available for download.
These override rules do not affect what is executed during judging.

#### Displayed test cases

For samples and test cases with `full_feedback` set to `true` (visible to the team only after a failure), the default displayed files are:
  - Input: `.in`
  - Output: `.ans`
...
Note that it is not allowed to have two files overriding the same display.

For the same base name, if `.in.statement` exist, then `.in.download` must also be present; likewise for `.ans.statement` and `.ans.download`.

#### Downloadable test cases

For files shown to teams, if the sample or test cases with `full_feedback` set to `true` (available to the team only after a failure), the default downloadable files are:
  - Input: `.in`
  - Output: `.ans`
  - The files in `.files`, if present
...
Note that it is not allowed to have two different files overriding `.ans`. If the provided `.in.download` is an empty file, then no input is available for download; likewise for `.ans.download`.

For the same base name, `.in.download` requires `.in.statement` or `.interaction`. Similarly, `.ans.download` requires `.ans.statement` or `.interaction`. `.interaction` is never itself downloadable and may exist with no `.download` files. 

Note that it is an error to provide `.out` together with `.ans.download` or `.interaction`. 
