...
This section specifies which files take precedence for display and download. The file with highest precedence that is present is the one that should be used. These priorities do not affect what is executed during judging unless explicitly noted.
...
For interactive and multi-pass problems, if `.interaction` exists, it is the only thing shown for the sample (and for any test case with full feedback). Do not provide `.in.statement`, `.ans.statement`, or `.out` alongside `.interaction`.
