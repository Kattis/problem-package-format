...
We allow customizing which data is used for each purpose with the additional extensions `.statement` and `.download`. See more in [Display and download precedence for test case files](#display-and-download-precedence-for-test-case-files).
...
This can be configured by using `.out`, `.in.statement`, and `.ans.statement` files.
...
By default, the `.in`, `.ans`, and the files in `.files` in `data/sample` are available for download. The files available for download can be configured by using `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files. See more in [Downloadable test cases](#downloadable-test-cases).
...
The exact files displayed and made available for download can be customized by using `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files. See more in [Displayed test cases](#displayed-test-cases). 
...
### Display and download precedence for test case files

This section summarizes which files take precedence for display and download.
Each list order defines the search order, and use the first file that exists.
These rules do not affect what is executed during judging.

#### Displayed test cases

For samples and test cases with `full_feedback` set to `true` (visible to the team only after a failure), the display precedence is:
  - Input shown: `.in.statement` > `.in`.
  - Output shown: `.ans.statement` > `.out` > `.ans`.
Note that it is an error to have both `.out` and `.ans.statement`. 

For interactive and multi-pass problems, if `.interaction` exists, it is the only thing shown for the sample (and for any test case with full feedback). It is an error to provide `.in.statement`, `.ans.statement`, or `.out` alongside `.interaction`.

#### Downloadable test cases

For files shown to teams, if the sample or test cases with `full_feedback` set to `true` (available to the team only after a failure), the download precedence is:
  - Input downloaded: `.in.download` > `.in`.
  - Output downloaded: `.ans.download` > `.out` > `.ans`.
The files in `.files` are included in downloads when present.
Note that it is an error to have both `.out` and `.ans.download`. 

For the same base name, `.in.download` and `.in.statement` must either both be present or both absent; likewise for `.ans.download` and `.ans.statement`.

For interactive and multi-pass problems, if `.interaction` exists, it is the only thing made available for download for the sample (and for any test case with full feedback). Do not provide `.in.download`, `.ans.download`, or `.out` alongside `.interaction`.
