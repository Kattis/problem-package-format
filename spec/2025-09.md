...
For samples and test cases with `full_feedback` set to `true`, the display precedence is:
...
For files shown to teams, if the sample or test cases with `full_feedback` set to `true` is available for download, the download precedence is:
