...
Each priority order defines the search order; use the first file that exists.
These priorities do not affect what is executed during judging unless explicitly noted.
...
For the files shown to the team, if the sample or test case with full-feedback is available for download, the priority is as follows.
