...
---------------------- | --------------------------------- | -----------
`data/sample/`         | [Samples](#samples)               | Sample test data
...
`.ans.statement`                | [Displayed test cases](#displayed-test-cases)       | Used instead of `.ans` in the statement
`.in.download`                  | [Downloadable test cases](#downloadable-test-cases) | Used instead of `.in` for downloads
`.ans.download`                 | [Downloadable test cases](#downloadable-test-cases) | Used instead of `.ans` for downloads
...
The `data/sample` directory contains test cases similar to those in `data/secret`, that are shared with solvers.
...
Note that samples are `full_feedback` by default.

If a test case is shared with the solver, then the following should be [displayed](#displaying-test-cases), and available for [download](#downloading-test-cases):
  - the given [input](#input),
  - the expected output.

If it is shared after the solver failed on the test case, then the following should also be displayed, and available for download:
...
If a test case input is overridden for display, it must also be overridden for [download](#dowloading-test-cases).
Same for output.

#### Downloading test cases

By default, when a test case is available for download, the `.in` (input) and `.ans` (output) files are used.
This can be configured by adding `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files.
