...
By default, the `.in`, `.ans`, and `.files` files in `data/sample` are available for download. This can customized by using `.out`, `.in.statement`, `.ans.statement`, `.in.download`, and `.ans.download` files. See more in [Downloaded test cases](#downloaded-test-cases).
